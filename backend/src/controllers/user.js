  ...
  try {
    ...
    if (verifyEmail.length > 0 && verifyEmail[0].userId) {
      ...
      return res.status(500).json({
        status: 500,
        message:
          "El correo proporcionado ya está asociado a otra cuenta de cliente, intenta con otro correo o inicia sesión con la cuenta asociada!",
      });
    }
    ...
    res.status(500).json({
      status: 500,
      message: "Error registrando al cliente con el correo: " + userEmail,
    });
  }
};
  ...
  try {
    ...
    if (verifyStatus.length > 0 && verifyStatus[0].userStatus !== "Active") {
      ...
      return res.status(403).json({
        status: 403,
        message:
          "Esta cuenta de cliente, se encuentra inhabilitada del sistema :(",
      });
    }
    // Find the password
